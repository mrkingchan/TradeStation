//
//  UILabel+NNHYBAttributeTextTapAction.m
//  DMHCAMU
//
...
@implementation NNHAttributeModel

@end

@implementation UILabel (NNHAttributeTextTapAction)
...
- (id<NNHAttributeTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setDelegate:(id<NNHAttributeTapActionDelegate>)delegate
{
    objc_setAssociatedObject(self, @selector(delegate), delegate, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

#pragma mark - mainFunction
- (void)nnh_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings tapClicked:(void (^) (NSString *string , NSRange range , NSInteger index))tapClick
{
    [self nnh_getRangesWithStrings:strings];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self nnh_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        
        if (weakSelf.tapBlock) {
            weakSelf.tapBlock (string , range , index);
        }
        
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(nnh_attributeTapReturnString:range:index:)]) {
            [weakSelf.delegate nnh_attributeTapReturnString:string range:range index:index];
        }
        
        if (self.isTapEffect) {
            
            [self nnh_saveEffectDicWithRange:range];
            
            [self nnh_tapEffectWithStatus:YES];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isTapAction) {
        if ([self nnh_getTapFrameWithTouchPoint:point result:nil]) {
            return self;
        }
    }
...
- (BOOL)nnh_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGAffineTransform transform = [self nnh_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self nnh_getLineBounds:line point:linePoint];
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                NNHAttributeModel *model = self.attributeStrings[j];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    if (self.isTapEffect) {
        ...
        [self performSelectorOnMainThread:@selector(nnh_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
...
- (CGAffineTransform)nnh_transformForCoreText
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)nnh_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
...
- (void)nnh_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void)nnh_saveEffectDicWithRange:(NSRange)range
{
...
- (void)nnh_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf nnh_getStringWithRange:range]];
            
            NNHAttributeModel *model = [NNHAttributeModel new];
...
- (NSString *)nnh_getStringWithRange:(NSRange)range
{
