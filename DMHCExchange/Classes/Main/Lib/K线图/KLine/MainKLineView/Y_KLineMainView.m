...
- (CGFloat)getExactXPositionWithOriginXPosition:(CGFloat)originXPosition{
    ...
    for (NSInteger index = startIndex > 0 ? startIndex - 1 : 0; index < arrCount; ++index) {
        ...
        if(xPositoinInMainView > minX && xPositoinInMainView < maxX) {
            if(self.delegate && [self.delegate respondsToSelector:@selector(kLineMainViewLongPressKLinePositionModel:kLineModel: longTouchPostionX:)]) {
                [self.delegate kLineMainViewLongPressKLinePositionModel:self.needDrawKLinePositionModels[index] kLineModel:self.needDrawKLineModels[index] longTouchPostionX:xPositoinInMainView];
            }
...
#pragma mark -  remove currentDropListView
