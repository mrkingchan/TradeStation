...
@property (nonatomic, strong) NNPriceVolumeView *priceVolumeView;  //悬浮价格成交量图
...
- (NNPriceVolumeView *)priceVolumeView{
    if (!_priceVolumeView) {
        _priceVolumeView = [NNPriceVolumeView new];
        _priceVolumeView.hidden = YES;
        [self addSubview:_priceVolumeView];
        [_priceVolumeView mas_remakeConstraints:^(MASConstraintMaker *make) {
...
- (Y_KLineMainView *)kLineMainView {
    ...
    //加载rightYYView
    self.priceView.backgroundColor = [UIColor backgroundColor];
    self.volumeView.backgroundColor = [UIColor backgroundColor];
    self.accessoryView.backgroundColor = [UIColor backgroundColor];
...
- (void)event_longPressMethod:(UILongPressGestureRecognizer *)longPress {
    ...
    if(longPress.state == UIGestureRecognizerStateEnded) {
        ...
        self.priceVolumeView.hidden = YES;
...
- (void)kLineMainViewLongPressKLinePositionModel:(Y_KLinePositionModel *)kLinePositionModel kLineModel:(Y_KLineModel *)kLineModel longTouchPostionX:(CGFloat)X
{
    //更新ma信息
//    [self.kLineMAView maProfileWithModel:kLineModel];
    [self removeCurrentDropView];
    [self refreshPriceVolumeViewWithkLineModel:kLineModel touchX:X];
    [self.volumeMAView maProfileWithModel:kLineModel];
    [self.accessoryMAView maProfileWithModel:kLineModel];
}

- (void)refreshPriceVolumeViewWithkLineModel:(Y_KLineModel *)model touchX:(CGFloat)X {
    self.priceVolumeView.hidden = NO;
    [self.priceVolumeView refreshWithModel:model];
    if (X>self.bounds.size.width /2.0) {
        [self.priceVolumeView mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.mas_top).offset(10);
            make.left.equalTo(self.mas_left).offset(10);
            make.width.equalTo(@115);
            make.height.equalTo(@145);
